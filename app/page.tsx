...
export default function DashboardPage() {
  ...
  // Warm theme chart colors
  const warmPieColors = ['#2D7A4D', '#C97A1E', '#A62C2C'];
    ...
    <>
      ...
      <div className="flex-1 p-6 md:p-8 space-y-8 max-w-7xl mx-auto w-full">
        ...
        >
          ...
          <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
            ...
            <div className="lg:col-span-7 space-y-4">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-bold" style={{ background: 'var(--accent-light)', color: 'var(--accent)' }}>
                <Shield className="w-3.5 h-3.5" />
                <span>{t('hero.tagline', 'IDENTITY INTELLIGENCE & FORENSICS')}</span>
              </div>
              ...
              <div className="pt-2 flex flex-wrap items-center gap-4">
                <Link
                  href="/verify"
                  className="btn-primary py-3.5 px-6 text-sm font-bold shadow-lg hover:scale-[1.02] transition-transform"
                >
                ...
                >
                  <span>{t('hero.viewHistory', 'View History')}</span>
                </Link>
              </div>
            ...
            <div className="lg:col-span-5 flex justify-center">
              ...
              >
                ...
                <div
                  className="absolute inset-x-0 h-1 bg-gradient-to-r from-transparent via-amber-500/60 to-transparent scan-line-subtle pointer-events-none"
                />
                ...
                <div className="flex items-center justify-between mb-4 pb-3 border-b" style={{ borderColor: 'var(--border-subtle)' }}>
                  ...
                  <span className="text-[10px] font-mono font-medium px-2 py-0.5 rounded" style={{ background: 'var(--accent-light)', color: 'var(--accent)' }}>
                    8 STAGES
                  </span>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center justify-between p-2.5 rounded-xl" style={{ background: 'var(--surface-warm)' }}>
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: 'rgba(200, 90, 50, 0.15)', color: 'var(--accent)' }}>
                        <FileCheck2 className="w-4 h-4" />
                      </div>
                      <div>
                        <p className="text-xs font-bold" style={{ color: 'var(--text-primary)' }}>Document Inspection</p>
                        <p className="text-[10px]" style={{ color: 'var(--text-muted)' }}>Passport • ID • License</p>
                      </div>
                    </div>
                  ...
                  <div className="flex items-center justify-between p-2.5 rounded-xl" style={{ background: 'var(--surface-warm)' }}>
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: 'rgba(217, 130, 43, 0.15)', color: 'var(--amber-accent)' }}>
                        <UserCheck className="w-4 h-4" />
                      </div>
                      <div>
                        <p className="text-xs font-bold" style={{ color: 'var(--text-primary)' }}>Face Match & Liveness</p>
                        <p className="text-[10px]" style={{ color: 'var(--text-muted)' }}>1:1 Biometric Comparison</p>
                      </div>
                    </div>
                    <span className="text-[10px] font-semibold text-emerald-700 dark:text-emerald-400">READY</span>
                  </div>
                  ...
                  <div className="flex items-center justify-between p-2.5 rounded-xl border" style={{ background: 'var(--accent-light)', borderColor: 'rgba(200, 90, 50, 0.3)' }}>
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[10px]" style={{ color: 'var(--text-muted)' }}>Multi-signal Evidence</p>
                      </div>
                    </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="p-4 rounded-xl border flex items-center gap-3 card-warm-subtle">
            ...
            <div>
              <p className="text-xs font-bold" style={{ color: 'var(--text-primary)' }}>94.2% Authenticity Rate</p>
          ...
          <div className="p-4 rounded-xl border flex items-center gap-3 card-warm-subtle">
            ...
            <div>
              <p className="text-xs font-bold" style={{ color: 'var(--text-primary)' }}>3 Documents in Review</p>
              <p className="text-[11px]" style={{ color: 'var(--text-muted)' }}>Require manual analyst sign-off</p>
            </div>
          </div>
          ...
          <div className="p-4 rounded-xl border flex items-center gap-3 card-warm-subtle">
            ...
            <div>
              ...
              <p className="text-[11px]" style={{ color: 'var(--text-muted)' }}>Real-time 8-stage ML pipeline</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard
            title={t('stat.totalVerifications', 'Documents Verified')}
            value={stats.totalVerifications}
            icon={ShieldCheck}
            variant="default"
            trend={{ value: 12, label: t('stat.vsLastWeek', 'vs. last week') }}
          />
          <StatCard
            title={t('stat.authentic', 'Likely Authentic')}
            value={stats.authenticCount}
            icon={CheckCircle}
            variant="low"
            trend={{ value: 8, label: t('stat.vsLastWeek', 'vs. last week') }}
          />
          <StatCard
            title={t('stat.review', 'Manual Review Needed')}
            value={stats.reviewCount}
            icon={Eye}
            variant="review"
            trend={{ value: 2, label: t('stat.vsLastWeek', 'vs. last week') }}
          />
          <StatCard
            title={t('stat.highRisk', 'High Risk Detected')}
            value={stats.highRiskCount}
            icon={AlertTriangle}
            variant="high"
            trend={{ value: -3, label: t('stat.vsLastWeek', 'vs. last week') }}
          />
        </div>

        {/* ─── Asymmetric Layout: Charts Row ─── */}
                ...
                <AreaChart data={stats.activityData}>
                  <defs>
                    ...
                    <linearGradient id="gradientHighRisk" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="0%" stopColor="#A62C2C" stopOpacity={0.35} />
                      <stop offset="100%" stopColor="#A62C2C" stopOpacity={0} />
                    </linearGradient>
                  </defs>
                  <XAxis
                    dataKey="date"
                ...
                <PieChart>
                  ...
                  >
                    {stats.riskDistribution.map((entry, index) => (
                      <Cell key={index} fill={warmPieColors[index % warmPieColors.length]} />
                    ))}
                  </Pie>
                  <Tooltip
                    contentStyle={{
                ...
                <div key={i} className="flex items-center justify-between text-xs">
                  <div className="flex items-center gap-2">
                    <div
                      ...
                      style={{ background: warmPieColors[i % warmPieColors.length] }}
                    />
                ...
                >
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2.5">
                      <p className="text-sm font-bold truncate group-hover:text-[#C85A32] transition-colors" style={{ color: 'var(--text-primary)' }}>
              ...
              <div className="space-y-2">
                <div className="p-2.5 rounded-xl flex items-start gap-2.5 text-xs card-warm-subtle">
                  ...
                  <div>
                    ...
                    <p className="text-[11px]" style={{ color: 'var(--text-muted)' }}>Record ID: vrf-005 (Score: 82)</p>
                  </div>
                </div>
                ...
                <div className="p-2.5 rounded-xl flex items-start gap-2.5 text-xs card-warm-subtle">
                  ...
                  <div>
                    ...
                    <p className="text-[11px]" style={{ color: 'var(--text-muted)' }}>Record ID: vrf-003 (Score: 58)</p>
                  </div>
                </div>
              </div>
            </div>
